...
<div class="am-g am-g-fixed">
    ...
    <div class="am-u-sm-9"> <!-- 左半 -->
        ...
        <div> <!-- 评论框 -->
            <h3>评论</h3>
            <form id="commentForm" class="am-form" method="POST" action="/Discuss?action=publish">
            ...
            <ul class="am-comments-list am-comments-list-flip" id="comment-ul">
                <c:forEach items="${discuss}" var="item">
                    <li class="am-comment ">
                        <a href="/User?action=userdetail&id=${item.getUserIdByUsername(item.username)}">
                        ...
                        <div class="am-comment-main"> <!-- 评论内容容器 -->
                            <header class="am-comment-hd">
                                ...
                                <div class="am-comment-meta"> <!-- 评论元数据 -->
                                    <a href="/User?action=userdetail&id=${item.getUserIdByUsername(item.username)}" class="am-comment-author">${item.username}</a> <!-- 评论者 -->
                        ...
                        <c:if test="${sessionScope.user.grade<1||sessionScope.user.username==item.username}">
                            <a class="am-icon-remove am-align-right"
                               href="/Discuss?action=delete&musicId=${requestScope.music.getId()}&userId=${item.getUserIdByUsername(item.username)}&time=${item.publishTime}">删除</a>
                        </c:if>
                    </li>
                </c:forEach>
            </ul>
